...
import type { EmailLevel } from "@/services/levels";
...
function normalizeText(t: string) {
    return t
        .trim()
        .replace(/^[^\w@]+|[^\w@]+$/g, "") // 去掉开头/结尾的标点（但保留 @）
        .replace(/\s+/g, " ")
        .toLowerCase();
}

export default function EmailSpotGame({ email, maxLives = 5, onLevelComplete }: Props) {
    const containerRef = useRef<HTMLDivElement | null>(null);

    // 将每个段落拆成 token 数组（保留空格/链接）
    const paragraphsTokens = useMemo(() => {
        return email.paragraphs.map((p) =>
            p.split(/(\s+|https?:\/\/[^\s]+|[^\s]+)/g).filter(Boolean)
        );
    }, [email]);
                        ...
                        <div style={{ lineHeight: 1.8 }}>
                            {tokens.map((tok, idx) => {
                                ...
                                return (
                                    <span
                                        key={idx}
                                        onClick={(e) => handleTokenClick(idx, e)}
                                        tabIndex={0}
                                        role="button"
                                        onKeyDown={(e) => {
                                        ...
                                        className={
                                            ...
                                            (isSelected ? "bg-yellow-300/70 border rounded-sm" : "hover:bg-yellow-100")
                                        }
                                        style={{ padding: "2px 3px", margin: "0 2px" }}
                                        title="点击以标记可疑"
                                    >
                                        {tok}
                                    </span>
